...
u16 RxCounter1 = 0; //接收字节数
bool is_RxFrame1 = FALSE; //FALSE表示没有收到一帧串口数据，TRUE表示收到一帧串口数据
...
********************************************************************************
*/
void Com2Init(COMX* pcom, u32 baund, u32 cfg)
{ 
    NVIC_InitTypeDef  NVIC_InitStructure;
    GPIO_InitTypeDef  GPIO_InitStructure;   
...
#if LIST_COM2_IN_NUM > 16   
    QueListInit(&QueCOM2In, Com2InBuffer, LIST_COM2_IN_NUM);
    pcom->pQueueIn = &QueCOM2In;
#else
    pcom->pQueueIn = NULL;
#endif
    ...
    NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = PREEMPRIO_USART2;
    NVIC_InitStructure.NVIC_IRQChannelSubPriority        = SUBPRIO_USART2;
    NVIC_InitStructure.NVIC_IRQChannelCmd                = ENABLE;  
    NVIC_Init(&NVIC_InitStructure);


    ComxConfig(USART2, baund, cfg);
...
#if LIST_COM2_OUT_NUM > 16  
    QueListInit(&QueCOM2Out, Com2OutBuffer, LIST_COM2_OUT_NUM);
    pcom->pQueueOut  = &QueCOM2Out;
#else
    USART_ITConfig(USART2, USART_IT_TXE, DISABLE);
    pcom->pQueueOut    = NULL;
#endif
...
********************************************************************************
*/
void Com4Init(COMX* pcom, u32 baund, u32 cfg)
{ 
    NVIC_InitTypeDef  NVIC_InitStructure;
    GPIO_InitTypeDef  GPIO_InitStructure;   
...
#if LIST_COM4_IN_NUM > 16   
    QueListInit(&QueCOM4In, Com4InBuffer, LIST_COM4_IN_NUM);
    pcom->pQueueIn = &QueCOM4In;
#else
    pcom->pQueueIn = NULL;
#endif
    ...
    NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = PREEMPRIO_UART4;
    NVIC_InitStructure.NVIC_IRQChannelSubPriority        = SUBPRIO_UART4;
    NVIC_InitStructure.NVIC_IRQChannelCmd                = ENABLE;  
    NVIC_Init(&NVIC_InitStructure);


    ComxConfig(UART4, baund, cfg);
...
#if LIST_COM4_OUT_NUM > 16  
    QueListInit(&QueCOM4Out, Com4OutBuffer, LIST_COM4_OUT_NUM);
    pcom->pQueueOut  = &QueCOM4Out;
#else
    USART_ITConfig(UART4, USART_IT_TXE, DISABLE);
    pcom->pQueueOut    = NULL;
#endif
