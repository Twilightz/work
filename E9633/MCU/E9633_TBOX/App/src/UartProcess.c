...
u8 version[11] = {0x82,0x00,0x04,'1','.','1','6'};
...
{ 
    MemReadData(PAGE_ADDR,&SNLength,1);
		...
		if(SNLength == 0xFFFF ||SNLength>32)
		{
				SNLength =0;
				SNack[2] = SNLength;
...
void getGsensordata(void)
{
	  if(lis331dlh_Init == MEMS_SUCCESS)
		...
		else if(lis331dlh_Init == MEMS_ERROR)
		{
			  data.AXIS_X  = 0;
			  data.AXIS_Y  = 0;
			  data.AXIS_Z  = 0;
...
void writeSN(PACKET* Precvpacket)
{
	  u16 writeSN[32];
		...
		if(Precvpacket->index == CurrentPacketLen && CurrentPacketLen >= 0)
		{
			 ...
			 if(SNLength<=32)
			 {
					FlashClean(PAGE_ADDR);
					MemWriteData(PAGE_ADDR,&SNLength,1);
					MemWriteSN(PAGE_ADDR+16,writeSN,SNLength);
