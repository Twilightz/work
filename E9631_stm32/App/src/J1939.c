...
{	  
	  u32 Pgn;
	  u32 FilterID = 0;
	  //RTU_DEBUG("J1939_SendPGN\r\n");
	
	  if(Handle == NULL)
			 return;

	  J1939_MESSAGE.Priority = Handle->Request.buf[1];
	  J1939_MESSAGE.PDUFormat = J1939_PF_REQUEST;
	  J1939_MESSAGE.DestinationAddress = Handle->Request.buf[3];
	  J1939_MESSAGE.DataLength = J1939_PGN_DATA_LENGTH;
		
	  J1939_MESSAGE.SourceAddress = Handle->Request.buf[4];
	  
	  J1939_MESSAGE.Data[0] = Handle->Request.buf[5];
		J1939_MESSAGE.Data[1] = Handle->Request.buf[6];
		...
		{
			  Pgn = ( Handle->Request.buf[6] << 8) | Handle->Request.buf[5];
				//RTU_DEBUG("Pgn=%x\r\n",Pgn);
		...
		#if 0
		else if(Handle->Request.BitField == 0x00)
		{
