...
void diagnostic_request(DiagnosticRequestHandle* Handle,DiagnosticResponseReceived  callback)
{
	  ...
	  if(Handle == NULL)
		...
		RTU_DEBUG("Handle->Request.BitField=%d\r\n",Handle->Request.BitField);
		ato32(&Handle->Request.buf[1],&canid);
		RTU_DEBUG("canid=%x\r\n",canid);
		//标准ID
		Handle->Request.Ide = Get_Id_type(canid);
		RTU_DEBUG("Handle->Request.Ide=%d\r\n",Handle->Request.Ide);
		if(Handle->Request.Ide == DIAGNOSTIC_CAN_ID_STANDARD)
		{
				canid = (canid >> 16) << 21;
		    Handle->Request.Id = canid;
		}//扩展ID
		else if(Handle->Request.Ide == DIAGNOSTIC_CAN_ID_EXTENDED)
		{
...
{	
     ...
     switch(response->Pid){
		 ...
		 c32toa(&response->Id,&Pidack[3]);
		 memcpy(&Pidack[7],response->DataField,pci+1);
		 senddatapacket(&COM3,Pidack,FIXED_LENGTH+CurrentLen+CHECKSUM);
}
		 ...
		 switch(Mode)
		 {
			 ...
			 case REQUEST_EMISSIONS_DTC_CLEAR+0x40:
				 	  CurrentLen = 8+4+1;
						...
						c32toa(&response->Id,&dtcack[3]);
						memcpy(&dtcack[7],response->DataField,8);
						senddatapacket(&COM3,dtcack,FIXED_LENGTH+CurrentLen+CHECKSUM);
				     break;
			  ...
			  FF_DL = (response->DataField[0]&0x0F<<12)|response->DataField[1];
				...
				switch(Mode)
				{
						case REQUEST_VEHICLE_INFORMATION+0x40:
		                  ...
		                  c32toa(&response->Id,&MFack[3]);
											memcpy(&MFack[7],response->DataField,8);
											senddatapacket(&COM3,MFack,FIXED_LENGTH+CurrentLen+CHECKSUM);
								}
								break;
				}
		 }
